...
public class Neo4JDbInserter {
    ...
    void createDb(ArrayList params, ArrayList fr, ArrayList tt) {
        ...
        ArrayList all_node = new FindAllNodesOfGraph().find(GraphInit.graphDb);
            ...
            for (int i = 0; i < fr.size(); i++)
            {
                ...
                for (int j = 0; j < tt.size(); j++) {
                    ...
                    if (!f.equals(t)) 
                    {
                        ...
                        if (!FindAllNodesOfGraph.temp.contains(f) || !FindAllNodesOfGraph.temp.contains(t)) {
                            if (!FindAllNodesOfGraph.temp.contains(f)) {

                                System.out.println("From node created");
                                FindAllNodesOfGraph.temp.add(f);
                            ...
                            if (!FindAllNodesOfGraph.temp.contains(t)) {
                                System.out.println("To node created");
                                FindAllNodesOfGraph.temp.add(t);
                        ...
                        {
                        conversation = fromnode.createRelationshipTo(tonode, RelTypes.from);
